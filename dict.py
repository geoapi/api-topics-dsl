...
def construct_dynamic_dsl1(topic_name,api_name,not_included_dict_topics,not_included_dict_apis):
    i = 0
    op = ''
    match_ = ''
...
def check_dsl_string_boolean1(dsl_string):
   # print(dsl_string)
  #  dsl_string = dsl_string.encode('utf-8')
  #  print(dsl_string)
#    dsl_string = dsl_string.decode().split(' ')#locally
...
def construct_dynamic_dsl_boolean_query1(obj):
    obj = deque(obj)
    query = obj.popleft()
...
#    dsl_string = dsl_string.decode().split(' ')#locally
  ...
  #  topic_dict_temp = []
    ...
    while (len(l) > 0 and l[0] not in op) :
        query = query + " " + l.popleft()
    if any(x in query for x in api_dict_temp):
        api_name_and.append(query)
    elif any(x in query for x in topics_dict_temp):
        topic_name_and.append(query)
    else:
    ...
    while (len(l) > 0):
        ...
        while (len(l) > 0 and l[0] not in op):
           ...
           seperat = ' '
        if getOp == "AND" or getOp == "And" or getOp == "and":
            if any(x in getKey for x in api_dict_temp):
                api_name_and.append(getKey)
            elif getKey in topics_dict_temp:
                topic_name_and.append(getKey)
            else:
                query_content.append(getKey)
        elif getOp == "OR" or getOp == "Or" or getOp == "or" :
            if any(x in getKey for x in api_dict_temp):
                api_name_or.append(getKey)
            elif any(x in getKey for x in topics_dict_temp):
                topic_name_or.append(getKey)
            else:
                query_content.append(getKey)
        elif getOp == "NOT" or getOp == "Not" or getOp == "not":
    ...
    collect_query_op.append(query_content)
    collect_query_op.append(api_name_and)
    collect_query_op.append(topic_name_and)
    collect_query_op.append(api_name_or)
    collect_query_op.append(topic_name_or)
